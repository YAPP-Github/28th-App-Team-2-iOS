...
- `MainTab`이나 `Root`와 같이 전체 피처들을 조립하는 껍데기 화면 코드는 피처 레이어가 아닌 최상위 **`App` 레이어** 내부에 작성되어 최종 런타임 조립(Composition Root)을 담당합니다. 현재 `RootFeature`는 저장 세션을 복원해 온보딩 또는 인증 완료 상태로 연결하며, 인증 완료 사용자는 `MainTab`에서 운세·토닥이·행운액션·마이페이지 피처를 사용합니다.
