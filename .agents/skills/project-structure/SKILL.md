...
4. **Tuist 템플릿 사용 및 예제 앱 활용**:
   - `Project.swift` 작성 시 기본 API 대신 `Project.makeFeature(...)` 혹은 `Project.makeCore(...)` 빌더를 사용한다.
   - `makeCore(name: "...", hasExample: true)` 설정을 통해 Core 모듈 역시 단독 컴포넌트 렌더링 확인을 위한 `{CoreModuleName}Example` 예제 앱 타겟을 띄울 수 있다. 예제 앱 소스 코드는 아키텍처 import 검사 대상에서 스킵되나, 물리적인 타겟 의존성은 오직 자기 본체 모듈로만 제한된다.
   - `makeFeature(name: "...")` 호출을 통해 Interface 타겟, Implementation 타겟, 그리고 Testing 타겟을 생성하고 자동으로 분리시킨다. (Interface 및 Testing 타겟 분리가 기본 사양으로 내장됨)
