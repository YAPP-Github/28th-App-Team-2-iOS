...
- **Core 레이어 내부**: Core 모듈 간 수평/상향 참조는 금지되나, 상위 Core(DesignSystem, NetworkCore) ➡️ 하위 Core(Model, Utils) 단방향 참조는 허용한다.
...
2. 구현 중 Feature 구현체 간 import, Core ➡️ Feature/App import를 만들지 않는다. (Core 내부는 상위 ➡️ 하위 단방향 참조만 허용)
...
- `network-core`: NetworkCore의 HTTP 계층을 구현하거나 Feature별 Client에서 사용하는 구조를 설계할 때 트리거됨 (범용 HTTP 책임, 도메인 Client 경계, App 조립 규칙 등)
