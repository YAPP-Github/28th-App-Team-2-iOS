...
- 화살표를 제외한 캡슐의 콘텐츠 인셋은 좌우 16pt, 상하 6pt입니다.
- 텍스트는 `Body3/Medium (14/20)`을 사용하고 줄 수 제한과 말줄임 없이 줄바꿈하며, 가운데 정렬합니다.
- Tooltip 말풍선의 최대 너비는 화면 양 옆 20pt 여백을 제외하고 100% 이며, 초과하는 메시지는 해당 너비 안에서 줄바꿈합니다.
- 화살표는 Figma가 제공한 원본 벡터 에셋을 사용합니다.
- 화살표 SVG는 형태만 담당하며, 캡슐과 화살표의 색상·투명도는 Figma의 `Primitive/Opacity/80` 토큰에 대응하는 `opacity80` 컬러 에셋으로 관리합니다.
